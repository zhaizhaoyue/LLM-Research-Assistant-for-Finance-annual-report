...
def answer_textual_or_mixed(
    ...
    """
    文本/混合问题回答主流程：
    1) 选 Top-K 命中 → 清洗/截断上下文
    2) 生成引用（≥1 条兜底）
    3) rule-based 拼接回答（兜底）
    4) LLM 总结（可开关）
    返回: final_answer, reasoning, citations
    """
    if not hits:
        return (
            f"根据当前可用的检索结果，无法给出充分的信息来回答：{query}。",
            "没有可用的命中结果。",
            [],
        )
    ...
    # 2) 引用（合并 + 去重 + 排序）
    citations = ensure_citations(
        merge_citations(cite_from_ctx, top_citations_from_hits(hits, k=3)), hits
    )
    citations = sort_citations(dedupe_citations(citations))
    ...
    reasoning = (
        "LLM 在限定上下文内做摘要整合（若失败或输出为空则回退到规则拼接）。"
    )
...
def _rule_based_answer(query: str, ctx_blocks: List[str]) -> str:
    """兜底规则拼接：取前 1-2 段，拼接为简短回答。"""
    if not ctx_blocks:
        return f"根据当前可用的检索结果，无法给出充分的信息来回答：{query}。"

    first = ctx_blocks[0].split("\n")[0].strip()
    second = ctx_blocks[1].split("\n")[0].strip() if len(ctx_blocks) > 1 else ""

    # 如果包含百分比/增长等词，保留更完整的句子
    key_tokens = ["%", "increase", "decrease", "growth", "decline"]
    if any(tok in first.lower() for tok in key_tokens):
        ans = first
    else:
        ans = f"{first} …… {second}" if second else first

    return ans


def _build_llm_prompt(query: str, ctx_blocks: List[str]) -> str:
    """生成 LLM Prompt，限定在上下文内回答。"""
    ctx = "\n\n---\n\n".join(ctx_blocks[:6])
...
- If multiple views exist, reconcile them; if insufficient, say explicitly: "Insufficient context."
- Keep any figures exactly as shown; do not convert units or currencies.
- Answer in Chinese if the question is in Chinese; otherwise answer in English.
